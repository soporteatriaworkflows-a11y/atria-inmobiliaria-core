...
export default function OwnerDashboardPage() {
  ...
  const rule = demoLiquidationInput.participationRules.find(
    (r) => r.participantId === firstParticipant.participantId && r.propertyId === null,
  );
    ...
    >
      <section className="grid gap-3 lg:grid-cols-3">
        ...
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-white/10 bg-gradient-to-br from-atria-indigo/50 to-atria-graphite p-4 shadow-card">
          <h3 className="self-start text-sm font-semibold text-atria-fog">Mi participación</h3>
          <Donut value={participacion} center={`${participacion}%`} tone="primary" caption="del total" />
        </div>
      </section>
      ...
      <SectionPanel className="bg-gradient-to-br from-atria-violet/15 to-atria-graphite">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-atria-fog">Tu información está protegida</h2>
      ...
      <section className="grid gap-2.5 md:grid-cols-2">
        <QuickAction href="/liquidacion" label="Ver mi cierre" helper="Distribución del periodo." icon="liquidacion" />
        <QuickAction href="/solicitudes" label="Pedir una revisión" helper="Solicita un cambio o aclaración." icon="solicitudes" />
      </section>
    </AppShell>
  );
}
