...
const activity = [
  {
    date: "Hoy",
    actor: "Contador",
    action: "revisó los gastos del periodo.",
    ...
    >
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          helper="Total de prueba del periodo."
          tone="success"
          icon="recaudos"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={String(demoLiquidationInput.properties.length)}
        ...
        <MetricCard
          label="Solicitudes"
          value="2"
          helper="Ajustes pendientes de revisión."
          tone="neutral"
          icon="solicitudes"
        />
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-3">
        <div className="flex flex-col gap-4 rounded-xl border border-atria-edge bg-atria-graphite p-4 shadow-card sm:p-5 lg:col-span-2">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-atria-mist">
                Periodo {demoLiquidationInput.month} · en revisión contable
              </p>
            </div>
          ...
          <div className="grid gap-1.5">
            <div className="flex items-baseline justify-between">
              <span className="text-xs font-medium text-atria-mist">
                Avance del cierre
              </span>
              <span className="text-xs font-semibold text-atria-lavender">
                {cierreProgreso}%
              </span>
            </div>
            <ProgressBar
              ariaLabel={`Avance del cierre: ${cierreProgreso}%`}
              value={cierreProgreso}
              tone="primary"
            />
          </div>
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-atria-edge bg-gradient-to-br from-atria-indigo/50 to-atria-graphite p-4 shadow-card sm:p-5">
          ...
          <Donut
            ...
            max={ingresos}
            tone="primary"
            center={`${Math.round((saldo / ingresos) * 100)}%`}
            caption="del ingreso"
          />
          <p className="text-lg font-semibold text-atria-fog">
            {formatCop(saldo)}
          </p>
          <p className="text-center text-2xs text-atria-mist">
            Disponible para distribución tras gastos.
          </p>
        </div>
      </section>
        ...
        <div className="rounded-xl border border-atria-edge bg-atria-graphite p-4 shadow-card sm:p-5 lg:col-span-2">
          ...
          <div className="grid gap-2.5 sm:grid-cols-2">
            <QuickAction
              ...
              helper="Pendientes y solicitudes."
              icon="admin"
            />
            <QuickAction
              href="/dashboard/contador"
              label="Revisar cifras"
              helper="Ingresos, gastos y cierre."
              icon="contador"
            />
            <QuickAction
              href="/herederos"
              label="Propietarios"
              helper="Participación y resúmenes."
              icon="participantes"
            />
            <QuickAction
              href="/liquidacion"
